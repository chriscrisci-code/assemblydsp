...
export async function createLicense(opts) {
  ...
  if (opts.source === "trial") {
    attempts.push({
      ...base,
      source: "manual",
      note: [base.note || "30-day free trial", `expires:${opts.expiresAt || plusDays(TRIAL_DAYS)}`]
        .filter(Boolean)
        .join(" | "),
    });
  }
...
/**
 * Mint a 30-day trial license. Optional email: one active trial per email.
...
export async function createTrialLicense(opts = {}) {
  ...
  const noteParts = [opts.note || "30-day free trial"];
...
/**
 * Admin license list. Does not return full keys.
...
export async function listLicenses() {
  ...
  const { data, error } = await supabase
    .from("licenses")
    .select(
      "id, product, source, email, note, status, max_activations, created_at, expires_at, license_key_last4",
    )
    .order("created_at", { ascending: false })
    .limit(250);
  ...
  const now = Date.now();
  return (data || []).map((row) => {
    const exp = resolveLicenseExpiresAt(row);
    const expired = Boolean(exp && Date.parse(exp) <= now);
    return {
      id: row.id,
      product: row.product,
      source: row.source,
      ...
      expiresAt: exp || row.expires_at || null,
      last4: row.license_key_last4,
      licenseType: isTrialLicense(row) ? "trial" : "paid",
    };
  });
}
