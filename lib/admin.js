...
import { createLicense, isLicenseConfigured, listLicenses, TRIAL_DAYS } from "./license.js";
...
export function isLocalAdmin() {
  return isAdminEnabled() && !process.env.VERCEL;
}

export function adminToken() {
  return createHmac("sha256", adminPassword() || "local-admin").update("assembly-dsp-admin").digest("hex");
}
...
export async function handleAdminRequest(req, res) {
  ...
  if (matchesAdminPath(pathname, "/api/admin/login") && req.method === "POST") {
    ...
    if (!body || !adminPassword() || !passwordsMatch(body.password)) {
  ...
  if (matchesAdminPath(pathname, "/api/admin/licenses") && req.method === "GET") {
    if (!isAdminRequest(req)) {
      sendJson(res, 401, { error: "Unauthorized." });
      return true;
    }
    ...
    try {
      ...
      sendJson(res, 200, { licenses, trialDays: TRIAL_DAYS });
    } catch (err) {
      console.error("[admin licenses]", err);
      sendJson(res, 500, { error: "Could not load licenses." });
    }
    return true;
  }

  if (matchesAdminPath(pathname, "/api/admin/license/grant") && req.method === "POST") {
    if (!isAdminRequest(req)) {
      sendJson(res, 401, { error: "Unauthorized." });
      return true;
    }
    ...
    const maxActivations = Number(body?.maxActivations ?? body?.max_activations ?? 2);
    try {
      const { license, licenseKey } = await createLicense({
        ...
        maxActivations:
          Number.isFinite(maxActivations) && maxActivations > 0
            ? Math.floor(maxActivations)
            : 2,
        product: body?.product || "chunk",
      });
      sendJson(res, 200, {
        ok: true,
        licenseKey,
      ...
      console.error("[admin license/grant]", err);
      sendJson(res, 500, { error: "Could not grant license." });
    }
    return true;
  }
