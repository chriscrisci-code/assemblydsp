...
import { TRIAL_DAYS } from "./license.js";
...
export async function sendLicenseKeyEmail(opts) {
  ...
  const build = await getCurrentBuild().catch(() => null);
  const versionLabel = build?.version ? ` v${build.version}` : "";
  const downloadLine = build?.filename
    ? `Download CHUNK${versionLabel} (${build.filename})`
    : `Download CHUNK${versionLabel}`;

  let expiryLine = "";
  if (isTrial) {
    const days = opts.trialDays || TRIAL_DAYS;
    if (opts.expiresAt) {
  ...
  const html = `
    ...
    <p><a href="${escapeHtml(downloadUrl)}">${escapeHtml(downloadLine)}</a></p>
